...
// Creating userMessages observable pipeline
const userMessages$ = submitEvents$.pipe(
    tap(event => event.preventDefault()),
    tap(x => console.log(x)),
    map(e => { const messageInput: HTMLInputElement = ((e.currentTarget as HTMLFormElement).querySelector('input[name="message"]')!);
